...
def test(args, model, test_edge_label_index, test_edge_label, epoch, lossVal_list, node_features, A, num_nodes):
    ...
    with torch.no_grad():
        # FastGTN
        # _, out = model(node_features, A, num_nodes, test_edge_label_index)
        # MLP
...
if __name__ == '__main__':
    ...
    if args.crossValidation == 1:
        ...
        for i in range(k):
            ...
            # 每次五折更新特征矩阵x
            fea5 = pd.read_csv('../feature_generate/feature05/disease_feature' + str(i) + '.csv', index_col=0)
            ...
            train_data = pd.read_csv('Siridataset05/DDI_train' + str(i) + '.csv', index_col=0)
            test_data = pd.read_csv('Siridataset05/DDI_test' + str(i) + '.csv', index_col=0)
            ...
            filename = f'./trained_model/student_trained_model/result_feature_05/final_cos_jl/KD_MLP_{i}.pth'
